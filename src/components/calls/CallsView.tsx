...
export const CallsView: React.FC<CallsViewProps> = ({
  ...
  return (
    <div className={`flex-1 flex flex-col h-full overflow-y-auto p-4 sm:p-6 max-w-4xl mx-auto w-full transition-colors duration-200 ${
      isDark ? 'bg-slate-950/60 text-white' : 'bg-slate-50 text-slate-900'
    }`}>
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className={`text-xl font-bold flex items-center gap-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>
            <Phone className="w-5 h-5" style={{ color: currentAccent.hex }} />
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-2xl font-bold text-xs shadow-lg transition hover:scale-105"
          style={{
